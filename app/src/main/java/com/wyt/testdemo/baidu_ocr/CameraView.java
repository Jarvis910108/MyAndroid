...
import com.wyt.testdemo.baidu_ocr.utils.ImageUtil;
import java.io.File;
...
public class CameraView extends FrameLayout {
    ...
    public static final int ORIENTATION_PORTRAIT = 0;//垂直方向
    public static final int ORIENTATION_HORIZONTAL = 90;//水平方向
    public static final int ORIENTATION_INVERT = 270;//水平翻转方向
    ...
    private ICameraControl cameraControl;//這是一個自定义的接口  把5.0之后的相机的类剥离出来了  单独管理
    private View displayView;//相机显示view
    private MaskView maskView;//身份证，银行卡，等裁剪用的遮罩
    private ImageView hintView;//用于显示提示证 "请对齐身份证正面" 之类的
    ...
    private void init() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            //5.0以上的處理方案  這個camera2  應該會返回一個抽象出來的接口
